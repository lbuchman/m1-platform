...
- Upload via the `az` CLI using an Azure Storage connection string. `scripts/publish-fw.sh` reads the connection string from `--destination-url`, or falls back to the `conString` in `provision-new-pc/azureStorage.json` if `--destination-url` is omitted. `azureStorage.json` is not committed to the repo (see `.gitignore`).
